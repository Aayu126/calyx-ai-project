...
def GoogleSearch(Topic):
    search(Topic) # Use pywnatkit's search function to perform a Google search.
...
def OpenApp(app, sess=requests.session()):
    try:
        ...
        if html:
            ...
            webopen(link) 
        return True # Open the link in a web browser.
...
if __name__ == "__main__":
    asyncio.run(Automation(["open youtube", "play talwinder famous song ", "open settings"  ]))
